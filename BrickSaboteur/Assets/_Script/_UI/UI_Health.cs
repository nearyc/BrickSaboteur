...
namespace BrickSaboteur
{
    ...
    public class UI_Health : ElementBaseSingle<UI_Health, IUITag>
    {
        ...
        protected override IEnumerator AfterStart()
        {
            ...
            var hpStream = MessageBroker.Default.Receive<Tag_ModifyHealth>();
            //非初始化
        ...
        private void ModifyHealth(int amount)
        {
            if (amount > 0)
            {
                ...
                {
                    BrickMgrM.LoaderManager.InstantiatePrefabByPath<GameObject>(AssetPath.HealthIcon, this.transform).Subscribe();
                }
            }
            else if (amount < 0)
            {
